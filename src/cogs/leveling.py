...
async def update_passive_xp():
    ...
    for member_tuple in memberID_list:
        ...
        async def update_xp_and_timestamp_updated(memberID, now, datetimeUpdate):
            ...
            xp = common.time_delta_to_hours(deltaTime) * common.passive_xp
    ...
    async def add_voice_xp(author_id):
        ...
        if addXPVoice:
            ...
            common.database.update([("memberID", author_id), ("timestampLastVoice", common.timestamp_convert(now))],
                                   dbTable="leveling")
            xp = common.time_delta_to_minutes(now - datetimeLastVoice) * common.voice_xp
...
class Leveling(commands.Cog):
    ...
    async def level(self, ctx, memberIDName=None):
        ...
        if not author:
            ...
            avatar = await author.avatar_url_as().read()
    ...
    async def update_level(self, author_id=None):
        async def level_check(memberID):
            achievements_channel = await self.bot.fetch_channel(common.achievements_channel_id)  # temporary for testing
            ...
            if new_level != level:
                ...
                await achievements_channel.send(f"Nice {member.name}, you are now at level {level}!")
